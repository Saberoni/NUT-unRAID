...
write_config() {
    ...
    if [ $MANUAL == "disable" ]; then
        ...
        if [ -n "$PORT" ]; then
            ...
            sed -i "3 s~.*~port = auto~" /etc/ups/ups.conf
        fi
        ...
        if [ $UPSKILL == "enable" ]; then
            var8='POWERDOWNFLAG /etc/ups/killpower'
            sed -i "3 s,.*,$var8," /etc/ups/upsmon.conf
        else
            var9='POWERDOWNFLAG /etc/ups/no_killpower'
        ...
        if [ -d $PLGPATH/ups ]; then
            if [ $( grep -ic "/etc/rc.d/rc.nut shutdown" /etc/rc.d/rc.6 ) -ge 1 ]; then
                cp -f /etc/ups/* $PLGPATH/ups
            else
                cp -f $PLGPATH/ups/* /etc/ups
            fi
        fi
    fi
